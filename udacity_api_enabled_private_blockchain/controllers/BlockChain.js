...
const LevelSandbox = require('../models/LevelSandbox.js');
const Block = require('../models/Block.js');

const blockDb = new LevelSandbox();
...
class Blockchain {
    ...
    async getBlockHeight() {
        try {
            let chainHeight = await blockDb.getDbBlockCount();
    ...
    async addBlock(block) {
        ...
        try {
            await blockDb.addDbBlock(block.height, JSON.stringify(block));
            // console.log("Successfully added block to the blockchain");
            return await this.getBlock(block.height);
        } catch (err) {
    ...
    async getBlock(height) {
        try {
            let requestedBlock = await blockDb.getDbBlockByHeight(height);
